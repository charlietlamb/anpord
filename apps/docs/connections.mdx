...
A run needs a credential for its [harness](/evals/harnesses) — the model usage is charged to your own account, so Anpord cannot supply one. Add it under **Settings > Harnesses** before the first run. This applies to the SDK, CLI, and MCP too; there is no way to pass a raw key through the API.
