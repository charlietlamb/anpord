...
Coding trials can take several minutes. `startAndWait` and `wait` poll for you; pass `onProgress` to watch cells advance. If no cell advances, inspect server logs and provider status.
