...
Hosted sandboxes can't reach services on your machine. To test against one, run the suite with `anpord eval --local`. See [local runs](/evals/local).
...
```ts
const { runs, total } = await anpord.evals.runs.list({ caseId: "writes-hello" });
```
