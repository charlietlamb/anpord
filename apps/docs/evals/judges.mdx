...
For direct OpenAI calls, replace `harness` with `provider: "openai"`. Add an OpenAI credential under **Settings > Judges**, or set `OPENAI_API_KEY` in the server and worker environment for a self-hosted deployment. A ChatGPT subscription is not an OpenAI API key.
