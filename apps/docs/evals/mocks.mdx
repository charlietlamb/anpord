...
`cli.path` is the executable. `command.path` is a non-empty argv segment tuple. Schema keys become kebab-case options; set an option's `name` to override it. String options accept `--id value` or `--id=value`. Boolean options are `true` when present and should be optional or have a schema default.
...
Definitions apply to every task in the suite. Split tasks into separate eval files when they need different interfaces. CLI mocks work with every harness; MCP mocks work with every [built-in harness](/evals/harnesses), not the custom command harness.
