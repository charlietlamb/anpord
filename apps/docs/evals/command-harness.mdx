...
The `command` harness runs a process you own instead of a coding agent Anpord installs. Anpord starts it in the sandbox, passes it the case through environment variables, reads events from its stdout, and scores the trial with the case's `validate`.
