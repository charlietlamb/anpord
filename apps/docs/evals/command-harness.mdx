...
Print one JSON object per line on stdout. A line with a known `_tag` is recorded and every other line is ignored, so ordinary logging is harmless. `at` is epoch milliseconds and optional. A line without it takes the moment it was read.
...
A non-zero exit does not fail the trial or discard earlier events. If the process omits `Finished`, Anpord records one with the reason `exit N`. The verifier scores the trial either way.
