...
Starting evals creates a batch with one run per case per variant. Each run holds its trials. `evals.start` returns the batch id and a run id for each case and variant while trials continue on the server. `startAndWait` resolves once the batch stops:

```ts
const batch = await anpord.evals.startAndWait(smoke);
```

To wait on a batch you started earlier, pass its id to `evals.wait({ id })`. Both poll `evals.get`, backing off as the batch goes on, and both take `onProgress`, `signal`, `timeoutMs`, `pollIntervalMs` and `maxIntervalMs`.

Neither cancels the batch. On timeout or abort it keeps going, and `evals.get` can still read it.
...
Code checks record their return value, context calls and results, logs and exceptions. Judges record the prompt, raw response, provider metadata and parsed judgment. Classifiers record the label they chose, the expected label and the probability of every option. Command verifiers record the command, stdout, stderr and exit code.
...
Each batch records a `trigger` (Dashboard, CLI, CI, API or MCP), and its runs inherit it. The CLI and MCP set it. Direct API calls default to API. Other automation can pass its own:
