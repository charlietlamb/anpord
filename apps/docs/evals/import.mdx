...
| `kind` | Becomes |
| --- | --- |
| `content_contains_any` | `answerContainsAny(needles)` |
| `content_contains_all` | `answerContainsAll(needles)` |
| `content_contains_none` | `answerContainsNone(needles)` |

Each scorer reads the agent's final answer from the trial journal. The
assertion's `text` stays above the call as a comment, so the generated line
still says what the author meant by it.
...
words, verbatim, next to a placeholder that scores zero:
