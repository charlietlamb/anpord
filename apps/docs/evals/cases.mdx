...
A suite is a group of cases that share a prompt, a workspace and a set of variants. Each case is one eval: it fills in the prompt, sets up the workspace and checks the result.
...
A suite's `id` is its `name` lowercased with hyphens, unless you set one. A case `id` is a handle of lowercase letters, digits and hyphens, unique within your organization. Its history is kept under it, so keep it stable. When a case's definition changes, later runs record a new version of the case.

| Case field | Purpose |
| --- | --- |
| `id` | Stable handle the case's history is kept under (required) |
| `name` | Label for the case's row in the grid (required) |
...
| `verify` | Shell check that runs after the agent |
| `validate` | Code validators, [model judges](/evals/judges), or an array of them |
...
Pass an array to combine checks, up to 20 per case: `validate: [hasHealthRoute, correctness]`. Use [model judges](/evals/judges) for checks that need interpretation.
...
`anpord eval` runs every `*.eval.ts` file it finds. Pass a path to run one: `anpord eval ./smoke.eval.ts`.

For one case, `evalCase()` takes the case fields plus `prompt`, `variants` and `trials`, and wraps them in a suite with the case's `id` and `name`.
