...
The first column is the stock harness. The second is your configured harness. Each is its own variant with its own runs, so profile changes are read against that profile's history.
...
The profile **name** is part of the variant. Two profiles on one base, one model and one sandbox are two variants with two histories, exactly as two harnesses would be.

The profile **version** is not. It is a content hash over the files, system prompt, environment, and install and run commands. An edited profile is a new run on the same variant, and each run records the version it used. Renaming a file changes the version. Renaming the profile starts a new variant.
...
Hold the model constant. Variants are not comparable when a profile requires an unavailable provider or the model ids do not overlap. State that beside the grid instead of interpreting the delta.
