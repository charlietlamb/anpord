...
| `env` | Environment variables, present for every process the trial starts |
| `install` | A command run once while the sandbox is prepared. Command harness only |
...
`install` and `run` belong to the [command harness](/evals/command-harness). A profile on any other base is rejected if it sets either.
...
Each base takes a system prompt in its own way. The prompt is written into the sandbox and handed to the harness by path:

| Base | Delivery |
| --- | --- |
| `claude` | `--append-system-prompt-file`, with `--add-dir` on the workspace so `CLAUDE.md` is discovered, plus `--settings` and `--mcp-config` when the profile ships them |
| `opencode` | `OPENCODE_CONFIG_CONTENT`, merging the profile's own config with the prompt's path in `instructions` |
| `codex` | `-c developer_instructions=…`, which adds to the built-in prompt rather than replacing it |
| `gemini`, `qwen`, `pi`, `fx`, `cursor` | Prepended to the instructions file the agent reads, `AGENTS.md` or `GEMINI.md` |
