...
Classifiers call TypeSafe directly. Configure `TYPESAFE_API_KEY` in the organization's environment connection, or in the server and worker environment for a self-hosted deployment. No harness or sandbox is involved, so no harness connection is consulted.
