...
Classifiers call TypeSafe directly and need their own credential, which [connections](/connections) covers. No harness or sandbox is involved, so no harness connection is consulted.

## Evidence and isolation

A classifier receives the rendered case prompt and the final answer. It does not receive the case workspace, transcript, MCP or CLI configuration, or sandbox environment.
...
Changing a classifier's prompt, options, or expectation changes the case definition, and later runs record the new version. Calibrate against known inputs before trusting a floor: a threshold set above the model's usual confidence fails every case, including the correct ones.
