...
const cases: StartEvalRequest["cases"] = [
  {
    name: "repairs add",
    variables: { task: "Fix the failing add function without changing its public API" },
    source: {
      kind: "repo",
...
Pin repository sources to a commit for reproducible runs. `startAndWait` polls until the grid finishes; pass `timeoutMs` to bound it, or `signal` to stop waiting on the job's own abort.
