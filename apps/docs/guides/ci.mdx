...
Under every gate, a batch that fails to start, fails, or times out fails the job.

The `report` output is the path to a JSON file with each suite file's batch ID, batch and problems. To keep it, upload it with `actions/upload-artifact` and `if: always()`.
...
The job is the PR check. For a separate check named `anpord` on the PR head commit, pass `GITHUB_TOKEN` in the step's `env` and grant `checks: write`. Its verdict follows the selected gate.
