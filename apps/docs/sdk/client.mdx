---
title: "API client"
description: "Submit evals, read results and manage prompts from TypeScript"
---
...
| `evals.startAndWait(request)` | The finished run |
| `evals.get({ id })` | One run with cells and trials |
| `evals.wait({ id })` | An already-started run once it finishes |
...
```ts
import { compileDefinition } from "anpord/eval";
import { smoke } from "./evals/smoke.eval";

const run = await anpord.evals.startAndWait(await compileDefinition(smoke));
```

`import.meta.url` tells Anpord where the eval was written, which is how the validators reach the sandbox with everything they close over. `compileEval("./evals/smoke.eval.ts")` takes a path instead and needs no argument to `defineEval`.
