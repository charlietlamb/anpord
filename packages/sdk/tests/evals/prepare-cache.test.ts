...
describe("what a case says it keeps", () => {
  ...
  test("reaches the payload the runner is given", async () => {
    const payload = await compiled(
      `{ id: "c", cache: { key: "deps-v1", path: "node_modules" }, name: "c", prepare: install, variables: { task: "x" }, validate: command("true") }`
    );
  ...
  test("is absent when the case keeps nothing", async () => {
    const payload = await compiled(
      `{ id: "c", name: "c", prepare: install, variables: { task: "x" }, validate: command("true") }`
    );
