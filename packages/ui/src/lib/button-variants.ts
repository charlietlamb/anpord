...
export const buttonVariants = cva(
  "group/button inline-flex shrink-0 select-none items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-clip-padding font-medium tracking-[-0.01em] outline-none transition-[color,background-color,border-color,box-shadow] duration-150 ease-out focus-visible:border-ring focus-visible:ring-4 focus-visible:ring-ring/20 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-4 aria-invalid:ring-destructive/20 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "skeleton:skeleton-block bg-foreground text-background shadow-(--shadow-control) hover:bg-foreground/90",
        outline: `skeleton:skeleton-block ${SURFACE_CONTROL} ${SURFACE_CONTROL_HOVER} text-foreground`,
        ghost:
        ...
        subtle: `skeleton:skeleton-block ${SURFACE_CONTROL} ${SURFACE_CONTROL_HOVER} text-muted-foreground hover:text-foreground aria-expanded:text-foreground`,
        raised:
