...
const repositoryWith = (initial: readonly ChannelCountRow[]) => {
  const rows = [...initial];
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ChannelCatalogLive.pipe(
          Layer.provide(
            Layer.mergeAll(
              Layer.succeed(ChannelRepository, repositoryWith(rows)),
...
describe("ChannelCatalog", () => {
  ...
  test("production cannot be renamed", async () => {
    const exit = await run([countRow("production", 1, "green")], (catalog) =>
  ...
  test("production cannot be deleted", async () => {
